...
def mostTweetedPeriod(filename,tags,splits):
	f = open(filename,'r')
	timeFile = open("goldenglobes.json","r")
	freqDict = defaultdict(int)
	#print filename,tags
	buildList = []
	count = 0
	for line in f:
		time = int(json.loads(timeFile.readline())["created_at"]["$date"])
		count = count + 1	
		print time
		tweet = json.loads(line)
		if len(tweet)>=2 and all(any(e in word for word in tweet) for e in tags):
			buildList.append((time,1))
			bigramList = bigrams(tweet)
			for i in bigramList:
				freqDict[i[0]+" "+i[1]] += 1
		else:
			buildList.append((time,0))
...
def splitBuildList(list1,splits):
	...
	while count < len(list1):
		sum1 = 0
		inttime = list1[count][0]
		while abs(list1[count][0]-inttime) < splits:
			count = count + 1
			if count >= len(list1):
				break
			sum1 = sum1 + list1[count][1]
		print sum1
...
if __name__ == "__main__":
	...
	question = ["best director", "best picture comedy", "best actor drama", "best picture drama"]
	
	porter = nltk.PorterStemmer()
	for q in question:
		tags = [porter.stem(i) for i in q.split()]
		Y=mostTweetedPeriod(filename,tags,10000)
		X = range(0,len(Y))
		plt.bar(X,Y)
